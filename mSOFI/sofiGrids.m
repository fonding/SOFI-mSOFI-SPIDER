...
%
% The coordinates refer to the pixel neighbourhood, which spans 4 pixels along the x and y axes (-1:2). The coordinates index into the 4*4 pixels neighbourhood. The
...
%Copyright ? 2012 Marcel Leutenegger et al, 蒫ole Polytechnique F閐閞ale de Lausanne,
%Laboratoire d'Optique Biom閐icale, BM 5.142, Station 17, 1015 Lausanne, Switzerland.
%
...
orders=orders(orders > 0 & orders < 9);                                    %限制到8阶SOFI？？？
hash=sum(pow2(1,orders-1));                                                %计算2^(orders - 1) 求和？
if nargin > 1 && all
   hash=hash + pow2(1,20);
   all=@partitions;
else
...
%
function parts=partitions(order,next,parts)                                %输入order信息。 Next = 0, parts = {{}}
if nargin < 3
   next=0;
   parts={{}};
end
...
%
function parts=products(order)                                             %输入order
